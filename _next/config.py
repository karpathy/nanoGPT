...
def load_toml(path: Path) -> AppConfig:
    ...
    if (t := data.get("train")) is not None:
        train = TrainExperiment(
            model=mm_model(t["model"]),
            data=mm_data(t["data"]),
            optim=mm_optim(t["optim"]),
            schedule=mm_sched(t["schedule"]),
            runtime=mm_runtime(t["runtime"]),
        )
    sample = None
    if (s := data.get("sample")) is not None:
        sample = SampleExperiment(
            runtime=mm_runtime(s["runtime"]),
            sample=mm_sample(s["sample"]),
        )
