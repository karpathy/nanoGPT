...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        transposed = [
            ...
            'attn.c_proj.weight', 'mlp.c_fc.weight',
            'mlp.c_proj.weight'
        ]
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        # see PaLM paper Appendix B for details. For minGPT the formula is simplified:
        # flops per token = 6 * config.n_layer * config.n_embd^2
        # then multiply by dt to get MFU (model flops utilization)
        N = self.get_num_params()
        L, H, Q = self.config.n_layer, self.config.n_head, self.config.n_embd
        T = self.config.block_size
        flops_per_token = 6 * L * Q * Q
        flops_per_fwdbwd = flops_per_token * T
        flops_per_iter = flops_per_fwdbwd * fwdbwd_per_iter
        mfu = flops_per_iter / (dt * 1e12) # convert to TFLOPS
        return mfu
