...
wandb_run_name = 'rm' # 'run' + str(time.time())
# data
...
def reward(sequence, t='pizza'):
  if t in enc.decode(sequence.tolist()):
    # print('hello')
...
optimizer = torch.optim.AdamW(model.model.lm_head.parameters(), lr=1e-3)
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        text = enc.decode(X[iter % eval_interval].tolist())
        print(f"reward pred: {reward_probs[0][-1]}", "input: ", text[:30])
        # text = text.split()
        # text[10:14] = 'pizza'
        # text = "".join(text)
        test_text = 'I love pizza' + 'a'*10000
        test_text_enc = torch.tensor(enc.encode(test_text)[:256]).unsqueeze(0)
        test_reward_probs, _ = model(test_text_enc.to(device))
