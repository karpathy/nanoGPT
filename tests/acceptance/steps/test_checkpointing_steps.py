...
@given(
    parsers.parse(
        "a checkpoint manager with keep_last {keep_last:d} and keep_best {keep_best:d}"
    ),
...
def save_three_best(manager: CheckpointManager, m1: float, m2: float, m3: float):
    for i, m in enumerate([m1, m2, m3]):
        ...
        manager.save_checkpoint(
            ckpt, "ckpt_best.pt", metric=m, iter_num=i, logger=None, is_best=True
        )
        time.sleep(0.01)
...
def assert_two_best(manager: CheckpointManager):
    ...
    assert any(
        "_00000001_0.900000.pt" in n or n.endswith("_0.900000.pt") for n in names
    )
    assert any(
        "_00000000_1.000000.pt" in n or n.endswith("_1.000000.pt") for n in names
    )
...
def simulate_first_improvement(manager: CheckpointManager):
    ...
    manager.save_checkpoint(
        ckpt, "ckpt_best.pt", metric=0.5, iter_num=0, logger=None, is_best=True
    )
    manager.save_checkpoint(
        ckpt, "ckpt_last.pt", metric=0.5, iter_num=0, logger=None, is_best=False
    )
...
def both_exist_iter0(manager: CheckpointManager):
    assert any(
        p.name.startswith("ckpt_best_00000000")
        for p in manager.out_dir.glob("ckpt_best_*.pt")
    )
    assert any(
        p.name.startswith("ckpt_last_00000000")
...
def simulate_improvement_iter10(manager: CheckpointManager):
    ...
    manager.save_checkpoint(
        ckpt, "ckpt_best.pt", metric=0.4, iter_num=10, logger=None, is_best=True
    )
...
def only_best_iter10(manager: CheckpointManager):
    assert any(
        p.name.startswith("ckpt_best_00000010")
        for p in manager.out_dir.glob("ckpt_best_*.pt")
    )
    assert not any(
        p.name.startswith("ckpt_last_00000010")
        for p in manager.out_dir.glob("ckpt_last_*.pt")
    )
