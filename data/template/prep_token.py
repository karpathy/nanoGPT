...
def train_sentencepiece_model(input_file, model_prefix, vocab_size):
    ...
    spm.SentencePieceTrainer.train(f'--num_threads={num_threads} --input={input_file} --model_prefix={model_prefix} --vocab_size={vocab_size} --model_type=bpe')
...
def main():
    ...
    parser.add_argument("--method", type=str, choices=["sentencepiece", "tiktoken", "char"], default="sentencepiece", help="Tokenization method")
    ...
    if args.method == "sentencepiece":
        ...
        train_ids, stoi, itos = encode_char_level(train_data)
        val_ids, _, _ = encode_char_level(val_data)
    ...
    # Token counts and export to bin files
